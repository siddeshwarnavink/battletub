...
export class Viewport {
    ...
    public center() {
        ...
        let center_x = GloablState.player.pos.x + (GloablState.config.size.char / 2);
        let center_y = GloablState.player.pos.y + (GloablState.config.size.char / 2);

        for (let key in GloablState.keys) {
