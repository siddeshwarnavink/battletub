...
export class Player {
    ...
    public draw() {
        ...
        const frame = (player.movement.moving) ? keys[player.movement.key].f[player.movement.frame] : keys[player.movement.key].f[1]
        const pos_x = Math.round(player.pos.x - viewport.x + (config.win.width / 2) - (viewport.width / 2))
        const pos_y = Math.round(player.pos.y - viewport.y + (config.win.height / 2) - (viewport.height / 2))
    ...
    public light(pos_x: number, pos_y: number) {
        ...
        const radius = 100
        const radialGradient = context.createRadialGradient(light_x, light_y, 0, light_x, light_y, radius)
        ...
        if (this.torch.lit) {
            for (let y = 0; y < config.tiles.y; y++) {
                for (let x = 0; x < config.tiles.x; x++) {
                    const distance = Math.sqrt((Math.pow(x - config.center.x, 2)) + (Math.pow(y - config.center.y, 2)))
    ...
    public move(x: number, y: number) {
        ...
        for (let i = 0; i <= 1; i++) {
            const tile = ((i == 0) ? map.data.layout[pos.y][new_pos.x] : map.data.layout[new_pos.y][pos.x]) - 1
