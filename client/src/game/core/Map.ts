    ...
    public draw() {
        ...
        for (let y = y_min; y < y_max; y++) {
            for (let x = x_min; x < x_max; x++) {
                ...
                let tile_x = Math.floor((x * config.size.tile) - viewport.x + (config.win.width / 2) - (viewport.width / 2));
                let tile_y = Math.floor((y * config.size.tile) - viewport.y + (config.win.height / 2) - (viewport.height / 2));
